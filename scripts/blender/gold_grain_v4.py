"""Gold grain v4 — fixed framing. Scale pile to fill frame properly."""
import bpy
import math
import os
import random
...
OUTPUT_PATH = os.path.join(PROJECT, "static/sprites/gold/grain_test_v4.png")
...
noise.inputs["Scale"].default_value = 3.0  # scaled down since objects are bigger
...
# === SCATTER — work in scene units where 1 unit ~ 1m ===
# Pile radius ~0.4m in scene, pellet radius 0.08-0.22 in scene units
# Camera will be set to frame this properly
num_pellets = 120
pile_radius = 0.4
...
for i in range(num_pellets):
    ...
    # A few bigger pellets for visual interest
    if random.random() < 0.15:
        size = random.uniform(0.14, 0.22)
...
# === CAMERA — framed to show pile at ~60% of frame ===
...
cam_obj.rotation_euler = (Vector((0, 0, target_z)) - cam_obj.location).to_track_quat('-Z', 'Y').to_euler()
cam_data.lens = 65  # wider lens to capture more of the pile
...
print(f"GOLD GRAIN V4 COMPLETE: {OUTPUT_PATH}")
